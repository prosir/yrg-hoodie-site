...
async function migrateOrders() {
  ...
  for (const order of orders) {
    ...
    await query(sql, [
      ensureNotUndefined(order.id),
      ensureNotUndefined(order.orderId),
      ensureNotUndefined(order.name),
      ensureNotUndefined(order.email),
      ensureNotUndefined(order.phone),
      ensureNotUndefined(order.address),
      ensureNotUndefined(order.color),
      ensureNotUndefined(order.colorName),
      ensureNotUndefined(order.size),
      ensureNotUndefined(order.delivery),
      ensureNotUndefined(order.notes),
      ensureNotUndefined(order.price),
      ensureNotUndefined(order.status),
      ensureNotUndefined(order.date),
      ensureNotUndefined(order.isCrew) || false,
      ensureNotUndefined(order.orderedFromSupplier) || false,
      ensureNotUndefined(order.trackingNumber),
      ensureNotUndefined(order.trackingSent) || false,
      ensureNotUndefined(order.quantity) || 1,
    ])
  }
...
async function migrateProducts() {
  ...
  for (const product of products) {
    ...
    await query(productSql, [
      ensureNotUndefined(product.id),
      ensureNotUndefined(product.name),
      ensureNotUndefined(product.slug),
      ensureNotUndefined(product.description),
      ensureNotUndefined(product.categoryId),
      ensureNotUndefined(product.price),
      ensureNotUndefined(product.colorName),
      ensureNotUndefined(product.active),
      ensureNotUndefined(product.featured),
      ensureNotUndefined(product.createdAt),
      ensureNotUndefined(product.updatedAt),
    ])

    // Insert product images
    if (product.images && product.images.length > 0) {
      for (const image of product.images) {
        await query("INSERT INTO product_images (productId, imageUrl) VALUES (?, ?)", [
          ensureNotUndefined(product.id),
          ensureNotUndefined(image),
        ])
      }
    }
    ...
    if (product.sizes && product.sizes.length > 0) {
      for (const size of product.sizes) {
        await query("INSERT INTO product_sizes (productId, name, stock) VALUES (?, ?, ?)", [
          ensureNotUndefined(product.id),
          ensureNotUndefined(size.name),
          ensureNotUndefined(size.stock),
        ])
      }
    }
    ...
    if (product.colors && product.colors.length > 0) {
      for (const color of product.colors) {
        await query("INSERT INTO product_colors (productId, color) VALUES (?, ?)", [
          ensureNotUndefined(product.id),
          ensureNotUndefined(color),
        ])
      }
    }
  }
...
async function migrateCategories() {
  ...
  for (const category of categories) {
    ...
    await query(categorySql, [
      ensureNotUndefined(category.id),
      ensureNotUndefined(category.name),
      ensureNotUndefined(category.slug),
      ensureNotUndefined(category.description),
      ensureNotUndefined(category.isClothing),
      ensureNotUndefined(category.active),
      ensureNotUndefined(category.createdAt),
      ensureNotUndefined(category.updatedAt),
    ])

    // Insert category sizes
    if (category.sizes && category.sizes.length > 0) {
      for (const size of category.sizes) {
        await query("INSERT INTO category_sizes (categoryId, size) VALUES (?, ?)", [
          ensureNotUndefined(category.id),
          ensureNotUndefined(size),
        ])
      }
    }
  }
...
async function migrateRides() {
  ...
  for (const ride of rides) {
    ...
    await query(sql, [
      ensureNotUndefined(ride.id),
      ensureNotUndefined(ride.title),
      ensureNotUndefined(ride.date),
      ensureNotUndefined(ride.time),
      ensureNotUndefined(ride.startLocation),
      ensureNotUndefined(ride.distance),
      ensureNotUndefined(ride.description),
      ensureNotUndefined(ride.image),
      ensureNotUndefined(ride.spots),
      ensureNotUndefined(ride.registered) || 0,
      ensureNotUndefined(ride.active),
      ensureNotUndefined(ride.accessCode),
      ensureNotUndefined(ride.requireAccessCode) || false,
    ])
  }
...
async function migrateParticipants() {
  ...
  for (const participant of participants) {
    ...
    await query(sql, [
      ensureNotUndefined(participant.id),
      ensureNotUndefined(participant.rideId),
      ensureNotUndefined(participant.firstName),
      ensureNotUndefined(participant.lastName),
      ensureNotUndefined(participant.email),
      ensureNotUndefined(participant.phone),
      ensureNotUndefined(participant.motorcycle),
      ensureNotUndefined(participant.comments),
      ensureNotUndefined(participant.registeredAt),
    ])
  }
...
async function migrateAlbums() {
  ...
  for (const album of albums) {
    ...
    await query(albumSql, [
      ensureNotUndefined(album.id),
      ensureNotUndefined(album.title),
      ensureNotUndefined(album.description),
      ensureNotUndefined(album.category),
      ensureNotUndefined(album.date),
      ensureNotUndefined(album.coverImage),
      ensureNotUndefined(album.createdAt),
      ensureNotUndefined(album.updatedAt),
    ])

    // Insert album images
    if (album.images && album.images.length > 0) {
      for (const image of album.images) {
        await query("INSERT INTO album_images (albumId, imageUrl) VALUES (?, ?)", [
          ensureNotUndefined(album.id),
          ensureNotUndefined(image),
        ])
      }
